...
class Table {
    ...
    protected function buildTitle(): void {
        ...
        $desloc = sizeof($this->colSpec) + 1;
        $this->output .= $this->verticalExternalBorderChar;
        $this->output .= str_pad('', $this->fullAvailableWidth - $desloc, $this->horizontalExternalBorderChar);
        $this->output .= $this->verticalExternalBorderChar;
        $this->output .= PHP_EOL;
        $this->output .= $this->verticalExternalBorderChar;
        $this->output .= str_pad($this->title, $this->fullAvailableWidth - $desloc, ' ', \STR_PAD_BOTH);
        $this->output .= $this->verticalExternalBorderChar;
        $this->output .= PHP_EOL;
    }
    ...
    protected function buildHorizontalHeaderSeparator(): string {
        ...
        foreach ($this->colSpec as $key => $spec) {
            $output .= str_pad('', $spec['width'] - $desloc, $this->horizontalHeaderBorderChar);
    ...
    protected function buildHorizontalExternalSeparator(): string {
        ...
        $desloc = sizeof($this->colSpec)-1;
        foreach ($this->colSpec as $key => $spec) {
            $output .= str_pad('', $spec['width'] - $desloc, $this->horizontalExternalBorderChar);
            if ($key !== array_key_last($this->colSpec)) {
//                $output .= $this->intersectionChar;
    ...
    protected function buildHorizontalInternalSeparator(): string {
        ...
        $desloc = sizeof($this->colSpec)-1;
        foreach ($this->colSpec as $key => $spec) {
            $output .= str_pad('', $spec['width'] - $desloc, $this->horizontalInternalBorderChar);
            if ($key !== array_key_last($this->colSpec)) {
                $output .= $this->intersectionChar;
            }
        }
...
//        $desloc = 2;
        foreach ($this->data as $index => $row) {
            foreach ($row as $name => $cell) {
                $width = $this->colSpec[$name]['width'] - $desloc; //subtrai porque cada célula tem um separador em cada lado
